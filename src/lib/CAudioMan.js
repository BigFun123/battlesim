...
import { Bus, EVT_SETVOLUME } from "./Bus.js";

export class CAudioMan {
    path = "assets/audio/";
    sounds = [];
    music = [];
    ambient = [];
    ...
    constructor(scene) {
        ...
        Bus.subscribe("play-3daudio", (data) => {
    ...
    play3d(name, mesh, volume, loop = false) {
        ...
        if (!sound) {
            sound = new Sound(name, this.path + name, this.scene, null, { loop: loop, autoplay: false, spatialSound: true, maxDistance: 2000 });
